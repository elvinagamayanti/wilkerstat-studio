...
import { fetchInsets, saveInset, deleteInset } from '../api'
...
export default function InsetView({ active }) {
  ...
  useEffect(() => {
    fetchInsets()
      .then((rows) => setSavedInsets(rows))
      .catch((err) => showToast(`Gagal memuat WS-Inset: ${err.message}`))
  ...
  const confirmSave = async () => {
    ...
    try {
      ...
      setSavedInsets((prev) => [...prev, saved])
      setViewNo(saved.no)
      setModalOpen(false)
  ...
  return (
    <div className={`view inset-view${active ? ' active' : ''}`}>
      ...
      <div className="inset-map-wrap">
        {group ? (
          <div>
            <div className={`sheet ${orientasi}`} ref={sheetRef}>
