...
import { exportSheetPNG, exportSheetPDF, safeFileName } from '../exportSheet'
import { fetchSavedMaps, saveMap } from '../api'
...
export default function TemplateView({ mode, active }) {
  ...
  // muat daftar peta tersimpan dari MySQL
  useEffect(() => {
    fetchSavedMaps()
      .then((rows) => setSavedDb(new Set(rows.map((r) => r.map_key))))
      .catch((err) => showToast(`Gagal memuat basis data: ${err.message}`))
  ...
  const onSaveDb = async () => {
    ...
    if (mode === 'WA' && desa) {
      ...
      payload = {
        map_key: `WA:${desa.properties.iddesa}`,
        mode: 'WA',
        idkec: kec.properties.idkec,
        iddesa: desa.properties.iddesa,
        label: lbl,
      }
    } else if (mode === 'WS' && slsGroup) {
      lbl = slsGroup.nmsls
      payload = {
        map_key: `WS:${slsGroup.idsls}`,
        mode: 'WS',
        idkec: kec.properties.idkec,
        iddesa: desa.properties.iddesa,
        idsls: slsGroup.idsls,
        label: lbl,
      }
    } else if (mode === 'WSS' && subsls) {
      lbl = slsGroup.nmsls + ' - ' + subsls.properties.kdsubsls
      payload = {
        map_key: `WSS:${slsGroup.idsls}`,
        mode: 'WSS',
        idkec: kec.properties.idkec,
        iddesa: desa.properties.iddesa,
        idsls: slsGroup.idsls,
        idsubsls: subsls.properties.idsubsls,
        label: lbl,
      }
    } else return
    ...
    try {
      ...
      setSavedDb((prev) => new Set(prev).add(payload.map_key))
  ...
  const onExport = async (type) => {
    const el = sheetWrapRef.current?.querySelector('.sheet')
    if (!el || exporting) return
    setExporting(true)
    showToast(type === 'png' ? 'Menyiapkan PNG…' : 'Menyiapkan PDF…')
    try {
      if (type === 'png') await exportSheetPNG(el, exportName() + '.png')
      else await exportSheetPDF(el, exportName() + '.pdf')
  ...
  const isReady = (key) => savedDb.has(key)
